...
- **Refined Roundness:** In accordance with the `/impeccable` guidelines and the Stitch design system, buttons, tags, and text inputs must be pill-shaped (`rounded-full`). Cards, terminal blocks, and larger containers should use moderate roundness (`rounded-xl` or `12px–16px`) to indicate interactivity without looking "insanely rounded" (avoiding 32px+ radii).
