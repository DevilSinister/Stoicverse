...
export default async function DashboardPage() {
  ...
  const [profileResult, tierResult, progressResult, lessonsResult, eventsResult, notificationsResult] = await Promise.all([
    ...
    supabase.from("lessons").select("id, title, description, sort_order, tiers(level, title)").eq("status", "published").or(`release_at.is.null,release_at.lte.${now}`).order("sort_order"),
