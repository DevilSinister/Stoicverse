...
export default async function CreatorCourseManagerPageV2() {
  ...
  const [profile, tier, courseRows, prerequisiteRows, videoRows, assetRows] = await Promise.all([
  ...
  if ([profile, tier, courseRows, prerequisiteRows, videoRows, assetRows].some((result) => result.error)) throw new Error("Unable to load creator courses.");
