...
export async function renderCommunityWorkspace({ nextPath, workspace, selectedChannelId }: CommunityWorkspaceOptions) {
  ...
  const { data: postRows, error: postError } = permittedChannelIds.length
    ? await supabase.from("posts").select("id,channel_id,body,image_url,is_pinned,created_at,profiles!posts_author_id_fkey(full_name),reactions(id)").in("channel_id", permittedChannelIds).eq("is_deleted", false).order("is_pinned", { ascending: false }).order("created_at", { ascending: false }).limit(100)
    : { data: [], error: null };
