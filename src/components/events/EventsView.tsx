...
function EventCard({ event, now, currentTier, pending, canManage, enrollmentAvailable, onEnroll, onAddRoomLink }: { event: EventRecord; now: number; currentTier: number; pending: boolean; canManage: boolean; enrollmentAvailable: boolean; onEnroll: (id: string) => void; onAddRoomLink: (event: EventRecord) => void }) {
  ...
  const canEnter = event.enrolled && tierAllowed && isLive && Boolean(event.zoomUrl);
  const roomLabel = !event.enrolled ? "Enroll to access room" : !tierAllowed ? `Unlock Tier ${event.minTier}` : canEnter ? "Join Zoom room" : !event.zoomUrl ? (isLive ? "Room link pending" : `Link revealed in ${timeUntil(event.startsAt, now)}`) : `Room opens in ${timeUntil(event.startsAt, now)}`;
