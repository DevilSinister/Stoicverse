...
export function CreatorEventsView({ events, enrollmentAvailable, currentTier, isMaster, memberName }: {
  events: EventRecord[]; enrollmentAvailable: boolean; currentTier: number; isMaster: boolean; memberName?: string;
}) {
  ...
  return (
    <AppShell active="Events" title="Creator Events" isMaster={isMaster} currentTier={currentTier} memberName={memberName} routeBase="/creator">
      <main className="mx-auto max-w-7xl p-4 md:p-8">
        <div className="mb-6 flex flex-col gap-4 border-b border-surgical-steel pb-6 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="font-label text-xs tracking-[0.14em] text-primary-container">CREATOR EVENTS</p>
            <h1 className="mt-2 font-headline text-2xl font-bold text-white md:text-3xl">Manage the live session calendar.</h1>
            <p className="mt-2 max-w-2xl font-body text-sm leading-relaxed text-on-surface-variant">Schedule events, update secure room links, and review what members can access without mixing those controls into user routes.</p>
          </div>
          <button type="button" onClick={() => setIsCreating(true)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-primary-container px-5 font-label text-xs font-semibold text-on-primary-fixed transition hover:brightness-110 focus-ring"><Plus size={16} /> Create event</button>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_16rem]">
          <div className="space-y-8">
            <CreatorEventList title="On the schedule" events={upcoming} now={now} currentTier={currentTier} pending={isPending} enrollmentAvailable={enrollmentAvailable} onEnroll={submitEnrollment} onAddRoomLink={setRoomLinkEvent} />
            {recent.length > 0 && <CreatorEventList title="Recently concluded" events={recent} now={now} currentTier={currentTier} pending={isPending} enrollmentAvailable={enrollmentAvailable} onEnroll={submitEnrollment} onAddRoomLink={setRoomLinkEvent} />}
          </div>
...
function CreatorEventList({ title, events, now, currentTier, pending, enrollmentAvailable, onEnroll, onAddRoomLink }: { title: string; events: EventRecord[]; now: number; currentTier: number; pending: boolean; enrollmentAvailable: boolean; onEnroll: (id: string) => void; onAddRoomLink: (event: EventRecord) => void }) {
  return <section><div className="mb-3 flex items-center gap-3"><h2 className="font-headline text-lg font-semibold text-white">{title}</h2><span className="h-px flex-1 bg-surgical-steel" /></div>{events.length ? <div className="grid gap-4 lg:grid-cols-2">{events.map((event) => <CreatorEventCard key={event.id} event={event} now={now} currentTier={currentTier} pending={pending} enrollmentAvailable={enrollmentAvailable} onEnroll={onEnroll} onAddRoomLink={onAddRoomLink} />)}</div> : <div className="border border-dashed border-surgical-steel p-6 font-body text-sm text-fog-muted">No sessions in this window.</div>}</section>;
}

function CreatorEventCard({ event, now, currentTier, pending, enrollmentAvailable, onEnroll, onAddRoomLink }: { event: EventRecord; now: number; currentTier: number; pending: boolean; enrollmentAvailable: boolean; onEnroll: (id: string) => void; onAddRoomLink: (event: EventRecord) => void }) {
