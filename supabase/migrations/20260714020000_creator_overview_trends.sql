...
begin
  ...
  with days as (
    select generate_series(
      date_trunc('day', period_start at time zone creator_timezone) at time zone creator_timezone,
      date_trunc('day', now() at time zone creator_timezone) at time zone creator_timezone,
      '1 day'::interval
    ) as d
  ),
  daily_revenue as (
    ...
    where status = 'succeeded' and paid_at >= period_start
    group by 1
  ),
  daily_new_members as (
    ...
    where status = 'active' and joined_at >= period_start
    group by 1
  ),
